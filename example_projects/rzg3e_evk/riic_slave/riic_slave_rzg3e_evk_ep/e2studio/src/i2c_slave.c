 ...
 * @param[IN] None
 * @retval    FSP_SUCCESS       riic master and slave driver opened successfully.
 ...
 *              If slave riic transaction failure occurs then it halts the application turning LED ON
 *              as sign of failure. Also displays failure messages in RTT.
 *  @param[IN]  None
 *  @retval     FSP_SUCCESS    On successfully riic master and slave Write/Read operation.
...
static fsp_err_t iic_slave_write(void)
{
    ...
    while( (I2C_SLAVE_EVENT_TX_COMPLETE != g_slave_event) || (I2C_MASTER_EVENT_RX_COMPLETE != g_master_event) )
    {
        ...
        else
        {
            ...
            {
                /* we have reached to a scenario where riic event not occurred */
...
static void toggle_led(void)
{
        ...
        R_IOPORT_PinWrite(g_ioport.p_ctrl,LED_2, LED_ON);
        R_BSP_SoftwareDelay(TOGGLE_DELAY, BSP_DELAY_UNITS_MILLISECONDS);
        R_IOPORT_PinWrite(g_ioport.p_ctrl,LED_2, LED_OFF);

        R_IOPORT_PinWrite(g_ioport.p_ctrl,LED_3, LED_ON);
        R_BSP_SoftwareDelay(TOGGLE_DELAY, BSP_DELAY_UNITS_MILLISECONDS);
        R_IOPORT_PinWrite(g_ioport.p_ctrl,LED_3, LED_OFF);
}
...
void set_led(bsp_io_level_t led_state)
{
    ...
    R_IOPORT_PinWrite(g_ioport.p_ctrl,LED_2, led_state);
    R_IOPORT_PinWrite(g_ioport.p_ctrl,LED_3, led_state);
}

/*******************************************************************************************************************//**
 * This function is called to do closing of external riic master and slave module using its HAL level API.
 * @brief     Close the RIIC Master and Slave module. Handle the Error internally with Proper RTT Message.
