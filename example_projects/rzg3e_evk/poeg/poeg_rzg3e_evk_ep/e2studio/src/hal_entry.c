...
#define POEG_TEST_CONTROL_PIN               (BSP_IO_PORT_03_PIN_01)
...
void hal_entry (void)
{
    ...
    /* Initialize the POEG6 */
    err = init_poeg6_module();
    /* Handle error */
    if(FSP_SUCCESS != err)
    {
        APP_ERR_PRINT("\r\nPOEG6 initialization for GTETRG pin trigger and software api failed\r\n");
        APP_ERR_TRAP(err);
    }
    else
    {
        APP_PRINT("POEG6 initialized successfully for GTETRG pin trigger and software api\r\n");
    }
    ...
    switch (converted_rtt_input)
    {
        ...
        case POEG_OUTPUT_DISABLE_SOFTWARE_API:
        {
            ...
            while(true)
            {
                ...
                {
                    APP_READ(rByte);
                    /* Disable POEG6 GPT output pins using POEG software API */
                    err = R_POEG_OutputDisable(&g_poeg6_ctrl);
                    break;
                }
            }
            /* Handle error */
...
/*******************************************************************************************************************//**
 * @brief       This function is used for POEG Channel6 callback to disable the IRQ and set the Mode flag
 * @param[in]   poeg_callback_args_t * p_arg
 * @return      None
 **********************************************************************************************************************/
void channel6_user_callback(poeg_callback_args_t * p_args)
{
    FSP_PARAMETER_NOT_USED(p_args);
    ...
    /* If the POEG6 cannot be reset, disable the POEG6 interrupt to prevent
     * it from firing continuously */
    R_BSP_IrqDisable(POEG_GROUP6_IRQn);
}
...
fsp_err_t reset_poeg_module(void)
{
    ...
    switch (converted_rtt_input)
    {
        ...
        case POEG_CHANNEL_6:
        {
            ...
            do
            {
                err = R_POEG_Reset(&g_poeg6_ctrl);
            } while (FSP_SUCCESS != err);

            APP_PRINT("\r\n RESET POEG 6 successful\r\n");
            R_IOPORT_PinWrite(&g_ioport_ctrl, LED_PIN_POEG6, BSP_IO_LEVEL_LOW);
