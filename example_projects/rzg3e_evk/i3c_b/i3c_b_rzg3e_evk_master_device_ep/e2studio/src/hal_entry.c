...
void hal_entry (void)
{
    ...
    /* Start dynamic address assignment for the single device using the CCC SETDASA command. */
    /* device_count = 0 as required by SETDASA (direct CCC) */
    err = R_I3C_B_DynamicAddressAssignmentStart(&g_i3c0_ctrl, I3C_ADDRESS_ASSIGNMENT_MODE_SETDASA, RESET_VALUE, 0);
    if (FSP_SUCCESS != err)
    {
