...
/*******************************************************************************************************************//**
 * @brief This function demos both R_SPI_B_Write().
 * @param[IN]   None
...
/*******************************************************************************************************************//**
 * @brief This function demo R_SPI_B_Read().
 * @param[IN]   None
 * @retval      FSP_SUCCESS                  Upon successful SPI Write and SPI Read
 * @retval      Any Other Error code apart from FSP_SUCCES  Unsuccessful Write and Read
 **********************************************************************************************************************/
...
static mx25l1235f_status_t mx25l1235f_spi_transfer_bulk(uint8_t const *tx_data, uint8_t *rx_data, uint32_t length)
{
    ...
    if (0 == tx_data)
    {
        ...
        if (FSP_SUCCESS != R_SPI_B_Read (&g_spi_master_ctrl, rx_data, length, SPI_BIT_WIDTH_8_BITS))
        {
    ...
    else if (0 == rx_data)
    {
        ...
        if (FSP_SUCCESS != R_SPI_B_Write (&g_spi_master_ctrl, tx_data, length, SPI_BIT_WIDTH_8_BITS))
        {
            return MX25L12835F_ERROR;
        }
    }
    else
    {
        ...
        if (FSP_SUCCESS != R_SPI_B_WriteRead (&g_spi_master_ctrl, tx_data, rx_data, length, SPI_BIT_WIDTH_8_BITS))
        {
            return MX25L12835F_ERROR;
        }
    }
