 ...
 * @{
 **********************************************************************************************************************/
 can_frame_t g_canfd_ch4_tx_frame;                      //CAN 4 transmit frame
 can_frame_t g_canfd_ch4_rx_frame;                      //CAN 4 receive frame

 can_frame_t g_canfd_ch1_tx_frame;                      //CAN 1 transmit frame
 can_frame_t g_canfd_ch1_rx_frame;                      //CAN 1 receive frame

 /* Variable to store rx frame status info of channel 4*/
 can_info_t can_ch4_rx_info =
 {
...
extern bool b_canfd_ch4_tx_complete ;
extern bool b_canfd_ch4_rx_complete ;
...
void canfd_operation(void)
{
    ...
    g_canfd_ch4_tx_frame.id = CAN_ID;
    g_canfd_ch4_tx_frame.id_mode = CAN_ID_MODE_EXTENDED;
    g_canfd_ch4_tx_frame.type = CAN_FRAME_TYPE_DATA;
    g_canfd_ch4_tx_frame.data_length_code = CAN_CLASSIC_FRAME_DATA_BYTES;
    g_canfd_ch4_tx_frame.options = ZERO;

    /* Update transmit frame data with message */
    memcpy((uint8_t*)&g_canfd_ch4_tx_frame.data[ZERO], (uint8_t*)&tx_data[ZERO], CAN_FD_DATA_LENGTH_CODE);

    APP_PRINT("\n 1. Transmission of data over classic CAN Frame\n");

    /* Transmission of data over classic CAN frame */
    can_write_operation(g_canfd_ch4_ctrl, g_canfd_ch4_tx_frame);
...
static void can_write_operation(canfd_instance_ctrl_t p_api_ctrl, can_frame_t can_transmit_frame)
{
    ...
    while ((true != b_canfd_ch4_tx_complete) && (true != b_canfd_ch1_tx_complete))
    {
        g_time_out--;
    ...
    /* Reset flag bit */
    if(true == b_canfd_ch4_tx_complete )
    {
        b_canfd_ch4_tx_complete = false;
    }
...
static void can_data_check_operation(void)
{
    ...
    else if(RESET_VALUE == strncmp((char*)&g_canfd_ch4_rx_frame.data[ZERO], (char*)&rx_data[ZERO], CAN_CLASSIC_FRAME_DATA_BYTES))
    {
        /* Cleaning receive frame */
        memset(&g_canfd_ch4_rx_frame.data[ZERO], NULL_CHAR, CAN_CLASSIC_FRAME_DATA_BYTES);

        APP_PRINT("\nReceived Acknowledgement for Classic CAN Frame transmission.\nCAN operation Successful. Data length = %d\n", g_canfd_ch4_rx_frame.data_length_code);
        APP_PRINT("\n 3. Data transmission over FD frame\n");

        /* Updating FD frame parameters for channel 4*/
        g_canfd_ch4_tx_frame.id = CAN_ID;
        g_canfd_ch4_tx_frame.id_mode = CAN_ID_MODE_EXTENDED;
        g_canfd_ch4_tx_frame.data_length_code = CAN_FD_DATA_LENGTH_CODE;
        g_canfd_ch4_tx_frame.options = CANFD_FRAME_OPTION_FD | CANFD_FRAME_OPTION_BRS;

        /* Fill frame data that is to be sent in FD frame */
        for( uint16_t j = 0; j < SIZE_64; j++)
        {
            g_canfd_ch4_tx_frame.data[j] = (uint8_t) (j + 1);
        }
    ...
    else if(RESET_VALUE == strncmp((char*)&g_canfd_ch4_rx_frame.data[ZERO], (char*)&rx_fd_data[ZERO], CAN_FD_DATA_LENGTH_CODE)) // acknowledgement for second transmission
    {
        /* Cleaning receive frame */
        memset(&g_canfd_ch4_rx_frame.data[ZERO], NULL_CHAR, CAN_CLASSIC_FRAME_DATA_BYTES);
...
void can_read_operation(void)
{
    ...
    if (b_canfd_ch4_rx_complete == true)
    {
        /* Get the status information for CAN transmission */
        err = R_CANFD_InfoGet (&g_canfd_ch4_ctrl, &can_ch4_rx_info);
        /* Handle error */
        if (FSP_SUCCESS != err)
        {
        ...
        if ((can_ch4_rx_info.rx_mb_status[0]) || (can_ch4_rx_info.rx_mb_status[1]) || (can_ch4_rx_info.rx_mb_status[2]))
        {
            /* Read the input frame received */
            err = R_CANFD_Read (&g_canfd_ch4_ctrl, ZERO, &g_canfd_ch4_rx_frame);
            /* Handle error */
            if (FSP_SUCCESS != err)
            {
                APP_ERR_PRINT("\nCAN Read API FAILED");
 ...
 * @retval     None
 **********************************************************************************************************************/
void canfd_ch4_callback(can_callback_args_t *p_args)
{
    switch (p_args->event)
    {
        case CAN_EVENT_TX_COMPLETE:
        {
            b_canfd_ch4_tx_complete = true;     // set flag bit
            break;
        }
        case CAN_EVENT_RX_COMPLETE:
        {
            b_canfd_ch4_rx_complete = true;     // set flag bit
            break;
        }
...
void canfd_deinit(void)
{
    fsp_err_t err = FSP_SUCCESS;
    /* Close CANFD channel */
    err = R_CANFD_Close(&g_canfd_ch4_ctrl);
    if (FSP_SUCCESS != err)
    {
